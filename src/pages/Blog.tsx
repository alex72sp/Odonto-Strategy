...
const Blog = () => {
  ...
  // ORDEM OBRIGATÓRIA: Geral, Franqueado, Geral, Franqueado, Geral, Franqueado, Geral, Geral, Geral
  const posts = [
    {
    ...
    },
    {
      ...
      category: "Marketing",
      date: "18 Mai, 2024",
      image: "https://images.pexels.com/photos/6627413/pexels-photo-6627413.jpeg?auto=compress&cs=tinysrgb&w=800"
    },
    {
      ...
      <section className="pt-32 pb-20 px-4 bg-[#0B6B4F] text-white">
        <div className="max-w-4xl mx-auto text-center">
          ...
          <p className="text-lg md:text-xl text-white/80 leading-relaxed">
            Conteúdo especializado em gestão de clínica odontológica e estratégias para aumentar o faturamento.
          </p>
        </div>
      </section>
      ...
      <section className="py-12 md:py-20 px-4">
        <div className="max-w-6xl mx-auto">
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 md:gap-8 mb-16">
            {posts.map((post, i) => (
              <Link key={i} to={`/blog/${post.slug}`} className="flex">
                <Card className="h-full border-none shadow-sm hover:shadow-md transition-all overflow-hidden group flex flex-col w-full">
                  ...
                  <CardHeader className="p-5">
                    <div className="flex justify-between items-center mb-3">
                      <Badge variant="secondary" className={`${post.category === 'Franqueados' ? 'bg-[#0B6B4F] text-white' : 'bg-[#EAF7F1] text-[#0B6B4F]'} hover:opacity-90 text-[10px] uppercase font-bold`}>
                        {post.category}
                      </Badge>
                      <span className="text-[10px] text-gray-400 uppercase font-bold">{post.date}</span>
                    </div>
                    <CardTitle className="text-lg md:text-xl font-bold text-[#0B6B4F] group-hover:text-[#2ECC71] transition-colors line-clamp-3">
                      {post.title}
                    </CardTitle>
                  </CardHeader>
                  <CardContent className="p-5 pt-0 flex-grow">
                    <p className="text-sm text-[#333] leading-relaxed line-clamp-3">{post.excerpt}</p>
                  </CardContent>
                </Card>
              </Link>
            ))}
          ...
          <div className="flex flex-col items-center text-center max-w-2xl mx-auto gap-6 bg-white p-8 md:p-12 rounded-3xl shadow-sm border border-gray-100">
            <h3 className="text-2xl font-bold text-[#0B6B4F]">Quer aplicar essas estratégias na sua clínica?</h3>
            <p className="text-gray-600">
              Nossa consultoria ajuda você a transformar conhecimento em resultados práticos e faturamento real através de uma gestão de clínica odontológica profissional.
            </p>
